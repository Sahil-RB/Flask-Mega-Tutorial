...
@app.before_request #called before any view function, so anytime user makes a request, last seen is updated
def before_request():
...
def index():
    ...
    prev_url = url_for('index', page=posts.next) \
...
@app.route('/user/<username>') # <> encloses dynamic part, username will be an argument
@login_required
def user(username):
...
def edit_profile():
    ...
    elif request.method == 'GET': #if its the first time loading the page, populate fields with existing Data
